...
function fall_animation(dt, duration, height)
    if dt > duration then
        return 0
    elseif dt < duration/2 then
        return (dt/(duration/2))*height
    elseif dt > duration/2 then
        dt = dt - duration/2
        return height - ((dt/(duration/2))*height)
    end
end
function love.load()
    Center = {
...
function love.update()
    if animation.running == true then
        ...
        Dude.position = fall_animation(animation.interval, 1, 20)
